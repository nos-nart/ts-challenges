...
To solve `21106-medium-zu-he-jian-lei-xing-combination-key-type`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type Combs<T extends any[]> = T extends [infer Head extends string, ...infer Tail extends string[]]
  ? `${Head} ${Tail[number]}` | Combs<Tail>
  : never
```

3. **Step 3: How It Works**:
Generates combinations of 2 keys from token array `T`.
