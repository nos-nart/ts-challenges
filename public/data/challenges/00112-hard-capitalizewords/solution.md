...
To solve `112-hard-capitalizewords`, we need to construct a type transformation that satisfies all test cases.
...
> = S extends `${infer Head}${infer Tail}`
  ? Uppercase<Prev> extends Lowercase<Prev>
    ? `${Uppercase<Head>}${CapitalizeWords<Tail, Head>}`
    : `${Head}${CapitalizeWords<Tail, Head>}`
  : S
```

3. **Step 3: How It Works**:
Detects word boundaries by checking if the previous character was a non-letter symbol.
