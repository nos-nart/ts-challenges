...
To solve `9286-medium-firstuniquecharindex`, we need to construct a type transformation that satisfies all test cases.
...
> = T extends `${infer Head}${infer Tail}`
  ? Prev extends `${string}${Head}${string}`
    ? FirstUniqueCharIndex<Tail, `${Prev}${Head}`, [...Count, 1]>
...
Finds the 0-based index of the first non-repeated character in string `T`.
