...
To solve `9898-medium-zhao-chu-mu-biao-shu-zu-zhong-zhi-chu-xian-guo-yi-ci-de-yuan-su`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type FindEles<T extends any[], Seen extends any[] = [], Acc extends any[] = []> = T extends [infer Head, ...infer Tail]
  ? Head extends Seen[number]
    ? FindEles<Tail, Seen, Acc>
    : Head extends Tail[number]
      ? FindEles<Tail, [...Seen, Head], Acc>
      : FindEles<Tail, [...Seen, Head], [...Acc, Head]>
  : Acc
```

3. **Step 3: How It Works**:
Filters elements from tuple `T` that appear exactly once.
