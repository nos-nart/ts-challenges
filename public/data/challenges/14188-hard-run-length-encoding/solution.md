...
To solve `14188-hard-run-length-encoding`, we need to construct a type transformation that satisfies all test cases.
...
```ts
namespace RLE {
  export type Encode<S extends string> = S
  export type Decode<S extends string> = S
}
```
