...
To solve `7561-extreme-subtract`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type Subtract<M extends number, S extends number> = number
```
