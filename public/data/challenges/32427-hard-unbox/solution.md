...
To solve `32427-hard-unbox`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type Unbox<T> = T extends Promise<infer R>
  ? Unbox<R>
  : T extends () => infer R
    ? Unbox<R>
    : T extends (infer R)[]
...
Recursively unwraps Promises, Arrays, and Functions.
