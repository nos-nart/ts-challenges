...
To solve `9616-medium-parse-url-params`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type ParseUrlParams<T extends string> = T extends `${string}:${infer Rest}`
  ? Rest extends `${infer Param}/${infer Tail}`
    ? Param | ParseUrlParams<Tail>
    : Rest
  : never
```

3. **Step 3: How It Works**:
Extracts route parameter names starting with `:`.
