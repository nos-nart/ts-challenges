...
To solve `5181-hard-mutable-keys`, we need to construct a type transformation that satisfies all test cases.
...
Identifies mutable keys by checking if picking the key without `readonly` is distinct from the `Readonly` pick.
