...
To solve `35045-medium-longest-common-prefix`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type LongestCommonPrefix<T extends string[], Prefix extends string = ''> = T extends [infer Head extends string, ...infer Tail extends string[]]
  ? Head extends `${Prefix}${infer Char}${string}`
    ? T extends [`${Prefix}${Char}${string}`, ...any[]]
      ? LongestCommonPrefix<T, `${Prefix}${Char}`>
      : Prefix
    : Prefix
  : Prefix
```
...
Finds the longest shared initial substring among array of strings.
