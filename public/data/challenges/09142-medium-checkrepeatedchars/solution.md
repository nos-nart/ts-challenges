...
To solve `9142-medium-checkrepeatedchars`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type CheckRepeatedChars<T extends string> = T extends `${infer C}${infer Rest}`
  ? Rest extends `${string}${C}${string}`
    ? true
...
Checks if any character in string `T` appears more than once.
