...
To solve `9384-hard-maximum`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type Maximum<T extends number[]> = T extends [] ? never : T[number]
```

3. **Step 3: How It Works**:
Finds maximum number in array `T`.
