...
To solve `2059-hard-drop-string`, we need to construct a type transformation that satisfies all test cases.
...
```ts
type StringToUnion<S extends string> = S extends `${infer C}${infer R}` ? C | StringToUnion<R> : never
type DropString<S extends string, R extends string, DropUnion = StringToUnion<R>> = S extends `${infer Head}${infer Tail}`
  ? Head extends DropUnion
    ? DropString<Tail, R, DropUnion>
    : `${Head}${DropString<Tail, R, DropUnion>}`
  : ''
```

3. **Step 3: How It Works**:
Drops all characters appearing in filter string `R`.
